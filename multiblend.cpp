...
void help() {
  ...
  printf(
      "                         X < 0: reduce number of blending levels by "
      "-x\n");
  printf(
      "   -d DEPTH              override automatic output image depth (8 or "
      "16)\n");
  ...
  printf(
      ...
      "default 75)\n");
  printf(
      "  --cache                cache input images to disk to minimise "
      "memory usage\n");
  printf(
      "  --save-seams <file>    Save seams to PNG file for external editing\n");
  printf(
      "  --no-output            Don't perform blend (for use with "
      "--save-seams)\n");
  ...
  printf("\n");
  printf(
      "Pass a single image as input to blend around the left/right "
      "boundary.\n");
  exit(0);
}
...
int main(int argc, char *argv[]) {
  ...
  printf("\n");
  printf(
      "multiblend v0.6.2 (c) 2014 David Horman        "
  ...
  for (i = 1; i < argc - 1; i++) {
    if (!strcmp(argv[i], "-d")) {
      ...
      g_crop = false;
    else if (!strcmp(argv[i], "--bigtiff"))
      g_bigtiff = true;
    else if (!strcmp(argv[i], "--bgr"))
      g_bgr = true;
    else if (!strcmp(argv[i], "--wideblend"))
      g_wideblend = true;
      ...
      g_timing = true;
    else if (!strcmp(argv[i], "--dewhorl"))
      g_dewhorl = true;

    else if (!strcmp(argv[i], "-w"))
      output(0, "ignoring enblend option -w\n");
    else if (!strncmp(argv[i], "-f", 2))
      output(0, "ignoring enblend option -f\n");
    else if (!strcmp(argv[i], "-a"))
      output(0, "ignoring enblend option -a\n");
    else if (!strcmp(argv[i], "--no-ciecam"))
      output(0, "ignoring enblend option --no-ciecam\n");
    else if (!strncmp(argv[i], "--primary-seam-generator", 24))
      ...
      if (strcmp(comp, "0") == 0)
        ...
        g_jpegquality = atoi(comp);
      else if (_stricmp(comp, "lzw") == 0)
      ...
      g_debug = true;
    else if (!strcmp(argv[i], "--saveseams") ||
             !strcmp(argv[i], "--save-seams"))
      g_seamsave_filename = argv[++i];
    else if (!strcmp(argv[i], "--loadseams") ||
             !strcmp(argv[i], "--load-seams"))
      g_seamload_filename = argv[++i];
    else if (!strcmp(argv[i], "--simpleseam") ||
             !strcmp(argv[i], "--simple-seam"))
      g_simpleseam = true;
    else if (!strcmp(argv[i], "--savemasks") ||
             !strcmp(argv[i], "--save-masks"))
      g_savemasks = true;
    else if (!strcmp(argv[i], "--saveoutpyramids"))
      g_save_out_pyramids = true;
    else if (!strcmp(argv[i], "--savexor") || !strcmp(argv[i], "--save-xor"))
      g_xor_filename = argv[++i];
      ...
      if (!(_stricmp(ext, "jpg") && _stricmp(ext, "jpeg"))) {
        if (g_compression != -1) {
          output(0,
                 "warning: JPEG output; ignoring TIFF compression setting\n");
          g_compression = -1;
        }
  ...
  if (!g_output_filename && !g_seamsave_filename)
