...
void trim8(void *bitmap, uint32 w, uint32 h, int bpp, int *top, int *left,
...
void trim16(void *bitmap, uint32 w, uint32 h, int bpp, int *top, int *left,
            int *bottom, int *right) {
  size_t p;
  int x, y;
...
void trim(void *bitmap, int w, int h, int bpp, int *top, int *left, int *bottom,
          int *right) {
  if (bpp == 8)
    trim8(bitmap, w, h, bpp, top, left, bottom, right);
  else
    trim16(bitmap, w, h, bpp, top, left, bottom, right);
}
...
void extract8(struct_image *image, void *bitmap) {
  ...
  image->binary_mask.rows =
      (uint32 *)malloc((image->height + 1) * sizeof(uint32));
...
void extract(struct_image *image, void *bitmap) {
  if (image->bpp == 8)
    extract8(image, bitmap);
  else
    extract16(image, bitmap);
}

#define NEXTMASK                   \
  {                                \
    mask = *mask_pointer++;        \
    maskcount = mask & 0x7fffffff; \
    mask = mask >> 31;             \
  }
#define PREVMASK                   \
  {                                \
...
void inpaint8(struct_image *image, uint32 *edt) {
  ...
  for (c = 0; c < g_numchannels; c++)
    chan_pointers[c] = (uint8 *)image->channels[c].data;
    ...
    while (x < image->width) {
      ...
      if (mask) {
        ...
        if (x == 0) {
          ...
          for (c = 0; c < g_numchannels; c++)
            chan_pointers[c][x] = chan_pointers[c][copy];
          edt_p[x++] = dist;
          maskcount--;
        }
...
void inpaint16(struct_image *image, uint32 *edt) {
  ...
  for (c = 0; c < g_numchannels; c++)
    chan_pointers[c] = (uint16 *)image->channels[c].data;
  ...
  for (y = image->height; y > 1; y--) {
    ...
    edt_p += image->width;
    for (c = 0; c < g_numchannels; c++)
      chan_pointers[c] += image->width;  // p[c]+=image->width;
    x = 0;
...
void load_images(char **argv, int argc) {
  ...
  for (i = 0; i < g_numimages; i++) {
    I.channels =
        (struct_channel *)malloc(g_numchannels * sizeof(struct_channel));
  ...
  for (i = 0; i < g_numimages; i++) {
    ...
    if (i > 0 && I.bpp != g_images[i - 1].bpp)
      die("can't handle a mix of 8bpp and 16bpp images!");
    ...
    if (tiff_xpos == -1 && tiff_ypos == -1) {
      ...
      if (geotiff_read(I.tiff, &(g_images[i].geotiff))) {
        I.xpos =
            (int)(g_images[i].geotiff.XGeoRef / g_images[0].geotiff.XCellRes);
        I.ypos =
            (int)(g_images[i].geotiff.YGeoRef / g_images[0].geotiff.YCellRes);
      }
    } else {
      if (tiff_xpos != -1 && tiff_xres > 0)
        I.xpos = (int)(tiff_xpos * tiff_xres + 0.5);
      if (tiff_ypos != -1 && tiff_yres > 0)
        I.ypos = (int)(tiff_ypos * tiff_yres + 0.5);
      if (g_xres == -1 || g_yres == -1) {
        g_xres = tiff_xres;
        g_yres = tiff_yres;
      } else if (g_xres != tiff_xres || g_yres != tiff_yres) {
        output(0, "warning: image resolution mismatch (%f %f/%f %f)\n", g_xres,
               g_yres, tiff_xres, tiff_yres);
      }
    }
    ...
    if (tiff_xpos <= 0 && tiff_ypos <= 0 && compression != 1) {
      ...
      if (minstripcount > 2) {
        ...
        if (I.last_strip == s - 1)
          I.last_strip =
              s;  // after loop, s==TIFFNumberOfStrips(I.tiff)-1 (i.e.
                  // last strip)
      }
    ...
    if (I.last_strip == TIFFNumberOfStrips(I.tiff) - 1)
      I.tiff_u_height -= rowsmissing;

    untrimmed_pixels = I.tiff_u_height * I.tiff_width;
    if (untrimmed_pixels > untrimmed_pixels_max)
      untrimmed_pixels_max = untrimmed_pixels;
  }

  temp_t = untrimmed_pixels_max
           << (g_images[0].bpp >>
               2);  // bad assumption unless 8-bit+16-bit is forbidden
  ...
  for (i = 0; i < g_numimages; i++) {
    ...
    for (s = I.first_strip; s <= I.last_strip; s++)
      pointer += TIFFReadEncodedStrip(I.tiff, s, pointer, -1);

    g_workwidth = max(g_workwidth, (int)(I.xpos + I.tiff_width));
    g_workheight = max(g_workheight, (int)(I.ypos + I.tiff_height));

    trim(untrimmed, I.tiff_width, I.tiff_u_height, I.bpp, &I.top, &I.left,
         &bottom, &right);

    I.xpos += I.left;
    I.ypos += I.top;
    I.width = right - I.left + 1;
    I.height = bottom - I.top + 1;
    ...
    if (g_caching) {
      for (c = 0; c < g_numchannels; c++) {
        ...
        if (fopen_s(&I.channels[c].f, I.channels[c].filename, "wb"))
          die("couldn't open channel file");
#else
      ...
      for (c = 0; c < g_numchannels; c++)
        if (!(I.channels[c].data = (void *)malloc(temp_t)))
