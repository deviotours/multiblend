...
void png_mask(int i) {
  ...
  png_set_IHDR(png_ptr, info_ptr, g_workwidth, png_height, 8,
  ...
  for (l = 0; l < g_levels; l++) {
    ...
    for (y = 0; y < h; y++) {
      ...
      while (x < w) {
        ...
        {
          m = *input++;
          for (j = 0; j < -ipixel.i; j++)
            ((uint8 *)g_line0)[x++] = (int)(m * 255 + 0.5);
        } else {
      ...
      if (y == 0)
        while (x < g_workwidth) ((uint8 *)g_line0)[x++] = 128;
  ...
  if (pixel.i < 0)  // rle sequence
  {
    ...
    if (readsofar == inwidth)  // was this the only pixel? (special case)
    {
      *(int32 *)&output[outp++] = -outwidth;
      output[outp++] =
          pixel.f *
          4;  // because output will be squashed with 3 other lines later
      return p;
    }
...
void shrink_mask(float *input, float **output_pointer, int inwidth,
  ...
  output_p += squash_lines((float *)g_line0, (float *)g_line0, (float *)g_line2,
                           &output[output_p], outwidth);
  lines_read = 2;
  ...
  for (y = 1; y < outheight; y++) {
    ...
    output_p += squash_lines((float *)g_line0, (float *)g_line1,
                             (float *)g_line2, &output[output_p], outwidth);
  ...
  for (y = 0; y < g_workheight; y++) {
    ...
    for (i = 0; i < g_numimages; i++) {
      if (p[i] + (g_workwidth) > size[i]) {
        size[i] += g_workwidth << 1;
        g_images[i].masks[0] =
            (float *)realloc(g_images[i].masks[0], size[i] * sizeof(float));
      }
    }
  }
...
void shrink_masks() {
  ...
  for (i = 0; i < g_numimages; i++) {
    ...
    for (l = 0; l < g_levels - 1; l++) {
      ...
      shrink_mask(g_images[i].masks[l], &g_images[i].masks[l + 1], w, h, ow,
                  oh);
