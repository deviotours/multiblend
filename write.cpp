...
void tiff_out() {
  ...
  for (i = 0; i < g_numimages; i++)
    g_images[i].binary_mask.pointer = g_images[i].binary_mask.data;
  ...
  if (g_xres != -1) {
    TIFFSetField(g_tiff, TIFFTAG_XRESOLUTION, g_xres);
    TIFFSetField(g_tiff, TIFFTAG_XPOSITION, (float)(g_min_left / g_xres));
  }
  if (g_yres != -1) {
    TIFFSetField(g_tiff, TIFFTAG_YRESOLUTION, g_yres);
    TIFFSetField(g_tiff, TIFFTAG_YPOSITION, (float)(g_min_top / g_yres));
  }
