#define NEXTiMASK(i)                           \
  {                                            \
    temp = *g_images[i].binary_mask.pointer++; \
    maskcount[i] = temp & 0x7fffffff;          \
    mask[i] = ~temp & 0x80000000;              \
  }
#define PREViMASK(i)                           \
  {                                            \
    temp = *--g_images[i].binary_mask.pointer; \
...
void load_seams() {
  ...
  if (pw != g_workwidth || ph != g_workheight)
    die("PNG dimensions don't match workspace!");
  if (pd != 8 || pc != PNG_COLOR_TYPE_PALETTE)
    die("Incorrect seam PNG format!");
  ...
  while (y >= 0) {
    ...
    for (i = 0; i < g_numimages; i++) {
      ...
      if (y >= g_images[i].ypos && y < g_images[i].ypos + g_images[i].height) {
        ...
        g_images[i].binary_mask.pointer =
            &g_images[i].binary_mask.data
                 [g_images[i].binary_mask.rows[y - g_images[i].ypos +
                                               1]];  // point to END of line
      } else {
        maskcount[i] = g_workwidth;
        masklimit[i] = g_workwidth;
      }
...
void simple_seam() {
  ...
  for (y = 0; y < g_workheight; y++) {
    for (i = 0; i < g_numimages; i++) {
      if (g_images[i].ypos < y || g_images[i].ypos + g_images[i].height >= y) {
        g_images[i].d = -1;
        continue;
      }
...
void seam() {
  ...
  if (!g_seamload_filename) {
    ...
    if (!g_simpleseam) {
      g_edt =
          (uint32 *)_aligned_malloc(g_workwidth * g_workheight * sizeof(uint32),
                                    0);  // if malloc fails fall back on dtcomp
    ...
    if (g_seamwarning)
      printf("WARNING: some image areas have been arbitrarily assigned\n");
  } else {
    load_seams();
