...
void save_out_pyramid(int c, bool collapsed) {
  ...
  png_set_IHDR(png_ptr, info_ptr, g_output_pyramid[0].w, png_height, 8,
      ...
      for (x = 0; x < g_output_pyramid[l].w; x++) {
        if (l == g_levels - 1 || collapsed) {
          if (g_workbpp == 8)
            ((uint8 *)g_line0)[x] = max(
                0,
                ((short *)g_output_pyramid[l].data)[p++] >>
                    ACCURACY);  // ACCURACY=&((short*)out_pyramid)[g_output_pyramid[l].offset];
          else
            ((uint8 *)g_line0)[x] =
                ((int *)g_output_pyramid[l].data)[p++] >>
                16;  // ACCURACY=&((short*)out_pyramid)[g_output_pyramid[l].offset];
        } else {
          if (g_workbpp == 8)
            ((uint8 *)g_line0)[x] =
                ((((short *)g_output_pyramid[l].data)[p++] << 2) +
                 (0x7f << ACCURACY) + (0x7f >> (8 - ACCURACY))) >>
                ACCURACY;  // ACCURACY=&((short*)out_pyramid)[g_output_pyramid[l].offset];
          else
            ((uint8 *)g_line0)[x] =
                ((int *)g_output_pyramid[l].data)[p++] >>
...
void hshrink(struct_level *upper, struct_level *lower) {
  ...
  int xlim = (upper->x1 >> 1) -
             lower->x0;  // xpos on lower when we need to wrap last pixel
...
void vshrink(struct_level *upper, struct_level *lower) {
  ...
  int ylim = (upper->y1 >> 1) -
             lower->y0;  // ypos on lower when we need to duplicate last rows
  int lps;
  __m128i eight;
  __m128i sse_tmp1;
  __m128i sse_tmp2;
  ...
  if (g_workbpp == 8) {
    for (x = 0; x < lower->pitch; x++)
      ((short *)lower->data)[lp++] =
          (tmp[x] + (tmp[x] << 1) + tmp[x + lower->pitch] + 8) >> 4;
    for (y = 1; y <= y_extra0; y++) {
      memcpy(&((short *)lower->data)[lp], lower->data, lower->pitch << 1);
      lp += lower->pitch;
    }
  } else {
    for (x = 0; x < lower->pitch; x++)
      ((int *)lower->data)[lp++] =
          (tmp[x] + (tmp[x] << 1) + tmp[x + lower->pitch] + 8) >> 4;
    for (y = 1; y <= y_extra0; y++) {
      memcpy(&((int *)lower->data)[lp], lower->data, lower->pitch << 2);
      lp += lower->pitch;
    }
  }
  ...
  if (g_workbpp == 8) {
    for (; y < ylim; y++) {
      ...
      for (x = 0; x < lower->pitch; x++)
        ((short *)lower->data)[lp++] = ((int *)g_line0)[x];
      a = b;
      b = c;
      c += lps;
    }
  ...
  if (g_workbpp == 8)
    for (x = 0; x < lower->pitch; x++)
      ((short *)lower->data)[lp++] =
          (tmp[x - lower->pitch] + tmp[x] + (tmp[x] << 1) + 8) >> 4;
  else
    for (x = 0; x < lower->pitch; x++)
      ((int *)lower->data)[lp++] =
          (tmp[x - lower->pitch] + tmp[x] + (tmp[x] << 1) + 8) >> 4;
  ...
  if (g_workbpp == 8) {
    for (; y < lower->h; y++) {
      memcpy(&((short *)lower->data)[lp],
             &((short *)lower->data)[lp - lower->pitch], lower->pitch << 1);
      lp += lower->pitch;
    }
  } else {
    for (; y < lower->h; y++) {
      memcpy(&((int *)lower->data)[lp],
             &((int *)lower->data)[lp - lower->pitch], lower->pitch << 2);
      lp += lower->pitch;
    }
  }
}
...
void hps(struct_level *upper, struct_level *lower) {
  ...
  if (g_workbpp == 8)
    inflate_line_short(&((short *)lower->data)[lp], (short *)g_line0,
                       upper->pitch);
  else
  ...
  for (y = 1; y < ylim; y++) {
    if (g_workbpp == 8) {
      inflate_line_short(&((short *)lower->data)[lp], (short *)g_line1,
                         upper->pitch);
      for (x = 0; x < sse_pitch; x++) {
        sse_mix =
            _mm_add_epi16(((__m128i *)g_line0)[x], ((__m128i *)g_line1)[x]);
      ...
      for (x = 0; x < sse_pitch; x++) {
        sse_mix =
            _mm_add_epi32(((__m128i *)g_line0)[x], ((__m128i *)g_line1)[x]);
...
void copy_channel(int i, int c) {
  ...
  for (y = 0; y < top->h - y_extra1; y++) {
    ...
    if (y == 0) {
      switch (mode & 2) {
        case 0:
          for (; y < y_extra0; y++) {
            memcpy(&((short *)top->data)[op],
                   &((short *)top->data)[op - top->pitch], top->pitch << 1);
            op += top->pitch;
          }
          break;
        case 2:
          for (; y < y_extra0; y++) {
            memcpy(&((int *)top->data)[op],
                   &((int *)top->data)[op - top->pitch], top->pitch << 2);
            op += top->pitch;
          }
          break;
      }
  ...
  switch (mode & 2) {
    ...
    case 2:
      for (; y < top->h; y++) {
        memcpy(&((int *)top->data)[op], &((int *)top->data)[op - top->pitch],
               top->pitch << 2);
        op += top->pitch;
      }
      break;
...
void mask_into_output(struct_level *input, float *mask, struct_level *output,
                      bool first) {
  int x, y;
  void *input_line;
  int count;
  ...
  if (y_extra < 0) {
    input_line =
        (void *)&((char *)input_line)[(-y_extra * input->pitch) << bpp_shift];
    y_extra = 0;
  }
  ...
  for (y = y_extra; y < ylim; y++) {
    ...
    while (x < xlim) {
      ...
      if (pixel.f == 0) {
        ...
        if (g_workbpp == 8)
          memcpy(&((short *)out_p)[x], &((short *)input_line)[x],
                 limcount << bpp_shift);
        else
          memcpy(&((int *)out_p)[x], &((int *)input_line)[x], limcount
                                                                  << bpp_shift);
        x += count;
        count = 0;
      } else {
        if (g_workbpp == 8)
          ((short *)out_p)[x] +=
              (int)(((int16 *)input_line)[x] * pixel.f + 0.5);
        else
  ...
  if (g_workbpp == 8) {
    ...
    for (y = 0; y < g_workheight; y++) {
      ...
      for (x = 0; x < g_workwidth; x++) {
        q = (((short *)top->data)[dp + x] + g_dither[dith_off + (x & 31)]) >>
            ACCURACY;
        if (q < 0)
          q = 0;
        else if (q > 255)
    ...
    if (RAND_MAX == 32767)
      for (i = 0; i < 1024; i++) g_dither[i] = rand() >> 7;
    else
      for (i = 0; i < 1024; i++) g_dither[i] = rand() >> 23;
...
void blend() {
  ...
  for (i = 0; i < g_numimages; i++) {
    ...
    for (l = 0; l < g_levels; l++) {
      ...
      if (l == 0) {
        PY(i, l).x0 =
            (g_images[i].xpos - 1) & ~1;  // -1, round down to nearest even
  ...
  for (l = 0; l < g_levels; l++) {
    ...
    g_output_pyramid[l].pitch =
        (g_output_pyramid[l].w + pitch_plus) & ~pitch_plus;
  ...
  for (l = 0; l < g_levels; l++) {
    if (g_workbpp == 8)
      g_output_pyramid[l].data =
          &((short *)out_pyramid)[g_output_pyramid[l].offset];
    else
      g_output_pyramid[l].data =
          &((int *)out_pyramid)[g_output_pyramid[l].offset];
  }
  ...
  for (c = 0; c < g_numchannels; c++) {
    for (i = 0; i < g_numimages; i++) {
      ...
      copy_channel(
          i,
          c);  // also frees channel when finished (unless input caching is on)
      ...
      timer.set();
      for (l = 0; l < g_levels; l++)
        mask_into_output(&PY(i, l), g_images[i].masks[l], &g_output_pyramid[l],
                         i == 0);
      mio_time += timer.read();
    }
    ...
    for (l = g_levels - 1; l > 0; l--)
      collapse(&g_output_pyramid[l], &g_output_pyramid[l - 1]);
