void pseudowrap_split() {
  ...
  for (c = 0; c < g_numchannels; c++)
    g_images[1].channels[c].data =
        malloc(g_images[1].width * g_images[1].height * bpp);

  for (y = 0; y < g_images[1].height; y++) {
    for (c = 0; c < g_numchannels; c++)
      memcpy(
          (uint8 *)g_images[1].channels[c].data + (y * g_images[1].width) * bpp,
          (uint8 *)g_images[0].channels[c].data +
              (y * g_workwidth + split) * bpp,
          g_images[1].width * bpp);

    if (y > 0)
      for (c = 0; c < g_numchannels; c++)
        memcpy((uint8 *)g_images[0].channels[c].data +
                   (y * g_images[0].width) * bpp,
               (uint8 *)g_images[0].channels[c].data + (y * g_workwidth) * bpp,
               g_images[0].width * bpp);
  }

  for (c = 0; c < g_numchannels; c++)
    g_images[0].channels[c].data =
        realloc(g_images[0].channels[c].data,
                g_images[0].width * g_images[0].height * bpp);
  g_numimages++;
